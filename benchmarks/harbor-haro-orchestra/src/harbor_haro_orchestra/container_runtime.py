"""Run the production Haro agent stack inside the Harbor task container.

Each provisioned identity is a full ``haro-acp`` → ``haro-agent`` →
``haro-dev-mcp`` process tree launched *inside* the task container — the same
binaries and the same MCP toolset (shell, file tools, the ``haro`` CLI on
PATH) that the desktop app gives a Haro agent. The harness stays outside:
...
# Container-side layout for the uploaded Haro stack.
REMOTE_ROOT = "/opt/haro"
...
class RuntimeLaunchError(RuntimeError):
    """Raised when a Haro agent process cannot be launched or exits early."""
...
class HaroContainerRuntime:
    """Launch one production Haro agent stack per identity in the container."""
    ...
    def __init__(
        ...
        *,
        ...
        haro_acp_binary: str = "haro-acp",
        haro_agent_binary: str = "haro-agent",
        haro_dev_mcp_binary: str = "haro-dev-mcp",
        haro_cli_binary: str = "haro",
        relay_gateway: str = "",
        ...
        self.haro_acp_binary = haro_acp_binary
        self.haro_agent_binary = haro_agent_binary
        self.haro_dev_mcp_binary = haro_dev_mcp_binary
        # Host build used for user/provisioning operations only:
        self.haro_cli_binary = haro_cli_binary
    ...
    async def run(
        ...
        *,
        ...
        if not workers:
            raise RuntimeLaunchError("Haro orchestration requires at least one worker")
        trial_dir = self.logs_dir / "haro"
        ...
        try:
            ...
            # The task arrives exactly as it would in production Haro: a
    ...
    async def _install_stack(self, environment: BaseEnvironment) -> None:
        ...
        uploads = {
            f"{REMOTE_BIN}/haro-acp": self.haro_acp_binary,
            f"{REMOTE_BIN}/haro-agent": self.haro_agent_binary,
            f"{REMOTE_BIN}/haro-dev-mcp": self.haro_dev_mcp_binary,
        }
    ...
    async def _launch_agent(
        ...
        *,
        ...
        command = (
            f"{shlex.quote(f'{REMOTE_BIN}/haro-acp')} </dev/null "
    ...
    def _agent_env(
        ...
        *,
        ...
        return {
            **endpoint.env,
            "HARO_RELAY_URL": trial.relay_ws_url,
            "HARO_PRIVATE_KEY": credential.nostr_secret_key,
            # Desktop parity: the GUI also sets NOSTR_PRIVATE_KEY on haro-acp
            # so haro-dev-mcp's shim can wire git auth/signing for the agent.
            "NOSTR_PRIVATE_KEY": credential.nostr_secret_key,
            "HARO_AUTH_TAG": credential.nostr_auth_tag,
            "HARO_ACP_AGENT_COMMAND": f"{REMOTE_BIN}/haro-agent",
            "HARO_ACP_AGENT_ARGS": "",
            "HARO_ACP_MCP_COMMAND": f"{REMOTE_BIN}/haro-dev-mcp",
            "HARO_ACP_CHANNELS": trial.channel_id,
            "HARO_ACP_SUBSCRIBE": "mentions",
            "HARO_ACP_RESPOND_TO": "anyone",
            "HARO_ACP_NO_MEMORY": "true",
            "HARO_ACP_SYSTEM_PROMPT_FILE": remote_prompt,
            "HARO_AGENT_PROVIDER": endpoint.provider,
            "HARO_AGENT_MODEL": credential.llm_endpoint,
            "HARO_AGENT_MAX_OUTPUT_TOKENS": str(
                agent_class.generation.max_output_tokens
            ),
            "HARO_AGENT_MAX_CONTEXT_TOKENS": str(
                agent_class.generation.context_window_tokens
            ),
            "HARO_AGENT_MAX_ROUNDS": str(
    ...
    async def _wait_for_done(
        ...
        while True:
            ...
            polls += 1
            messages = await self._haro_json(
                trial.user, trial,
    ...
    # -- Haro CLI as the trial user / provisioning identities -------------------
    ...
    async def _haro_json(
        self, credential: AgentCredential, trial: TrialHandle, *args: str
    ) -> Any:
        process = await asyncio.create_subprocess_exec(
            self.haro_cli_binary,
            *args,
            stdout=asyncio.subprocess.PIPE,
            stderr=asyncio.subprocess.PIPE,
            env={
                **os.environ,
                "HARO_RELAY_URL": self._user_relay_url(trial),
                "HARO_PRIVATE_KEY": credential.nostr_secret_key,
                "HARO_AUTH_TAG": credential.nostr_auth_tag,
            },
        )
        ...
        if process.returncode != 0:
            raise RuntimeLaunchError(
                f"haro {shlex.join(args)} exited {process.returncode}: "
                f"{stderr.decode(errors='replace').strip()}"
            )
        try:
            return json.loads(stdout)
        except json.JSONDecodeError as error:
            raise RuntimeLaunchError("haro returned invalid JSON") from error
    ...
    def _user_relay_url(trial: TrialHandle) -> str:
        ...
        return HaroContainerRuntime._cli_relay_url(trial.relay_ws_url)
    ...
    def _compose_system_prompt(
        ...
        *,
        ...
        The analogue of a production Haro workspace's team context: each agent
        ...
        lines = [
            ...
            f"The team coordinates in Haro channel `{trial.channel_id}`.",
    ...
    def _reject_identity_overrides(endpoint: EndpointLaunchConfig) -> None:
        forbidden = {
            ...
            "HARO_AUTH_TAG",
            "HARO_ACP_CHANNELS",
            "HARO_ACP_MCP_COMMAND",
            "HARO_ACP_AGENT_COMMAND",
        }
