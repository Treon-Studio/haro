# Harbor Haro Orchestra

A stock-Harbor custom agent that runs a manifest-defined team through the real
Haro stack. Harbor sees one `HaroOrchestraAgent`; behind that adapter, one
...
`haro-acp` → `haro-agent` → `haro-dev-mcp` process tree the desktop app
launches: the production MCP toolset (shell, file tools, todo) with the
`haro` CLI on the shell's PATH. No Harbor fork or patch is required.
...
```bash
uv run --project benchmarks/harbor-haro-orchestra/testbed harbor run --yes -p <TASK_OR_DIRECTORY> --agent harbor_haro_orchestra:HaroOrchestraAgent --agent-kwarg manifest=<CONDITION.yaml> --agent-kwarg provisioner_factory=harbor_haro_testbed:provisioner_from_dict --agent-kwarg provisioner_config=<PROVISIONER.json> --agent-kwarg endpoint_config=<ENDPOINTS.json> --agent-kwarg artifact_root=benchmarks/harbor-haro-orchestra --agent-kwarg haro_acp_binary=<LINUX_BIN>/haro-acp --agent-kwarg haro_agent_binary=<LINUX_BIN>/haro-agent --agent-kwarg haro_dev_mcp_binary=<LINUX_BIN>/haro-dev-mcp --agent-kwarg haro_cli_binary=target/debug/haro --agent-kwarg run_id="bench-$(date -u +%Y%m%dT%H%M%SZ)" --agent-timeout-multiplier 15 --n-concurrent 1
```

`haro_acp_binary`/`haro_agent_binary`/`haro_dev_mcp_binary` must be **Linux**
...
any Linux base image works). `haro_cli_binary` is the **host** CLI the harness
...
Each trial gets fresh keys and a private Haro channel. The provisioner archives
rather than deletes that channel, leaving the relay/Postgres event timeline
and the per-agent acp/agent logs (downloaded into the trial's `haro/`
artifacts) available for analysis.

## Leaderboard runs

`just benchmark` is the one-command path: it stands up a dedicated Docker
stack (`haro-benchmark` compose project — relay :3600, Postgres :5633, secrets
...
relay membership list and opens the Haro desktop app logged in as them, so
...
uv run --project benchmarks/harbor-haro-orchestra/testbed \
    benchmarks/harbor-haro-orchestra/scripts/run_leaderboard.py \
    --dataset terminal-bench/terminal-bench-2-1 \
    --attempts 5 \
    --manifest benchmarks/harbor-haro-orchestra/manifests/<TEAM>.yaml \
    --endpoint-config benchmarks/harbor-haro-orchestra/testbed/endpoints/<ENDPOINTS>.json \
...
```bash
cd benchmarks/harbor-haro-orchestra
