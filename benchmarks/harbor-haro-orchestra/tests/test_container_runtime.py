...
from harbor_haro_orchestra.manifest import ExperimentManifest
from harbor_haro_orchestra.provisioning import AgentCredential, TrialHandle
from harbor_haro_orchestra.container_runtime import (
    REMOTE_BIN,
    REMOTE_LOGS,
...
def test_prompt_hash_and_identity_override_are_fail_closed(tmp_path):
    ...
    endpoint = EndpointLaunchConfig(
        "anthropic", "ANTHROPIC_API_KEY", {"HARO_ACP_MCP_COMMAND": "evil"}
    )
...
async def test_install_stack_uploads_the_pinned_stack(tmp_path):
    binaries = {}
    for name in ("haro-acp", "haro-agent", "haro-dev-mcp"):
        path = tmp_path / name
        path.write_text("#!binary")
        binaries[name] = str(path)
    rt = runtime(
        tmp_path,
        haro_acp_binary=binaries["haro-acp"],
        haro_agent_binary=binaries["haro-agent"],
        haro_dev_mcp_binary=binaries["haro-dev-mcp"],
    )
    environment = Environment()
    await rt._install_stack(environment)
    assert {target for _, target in environment.uploads} == {
        f"{REMOTE_BIN}/haro-acp",
        f"{REMOTE_BIN}/haro-agent",
        f"{REMOTE_BIN}/haro-dev-mcp",
    }
...
async def test_install_stack_requires_binaries_on_disk(tmp_path):
    rt = runtime(tmp_path, haro_acp_binary=str(tmp_path / "missing"))
...
async def test_forwarder_bridges_the_canonical_relay_address(tmp_path):
    from harbor_haro_orchestra.container_runtime import FORWARDER
...
async def test_launch_wires_the_desktop_environment(tmp_path, configured, expected):
    ...
    environment = Environment(
        responses={"haro-acp": ExecResult(stdout="4242\n", stderr="", return_code=0)}
    )
    ...
    assert f"{REMOTE_BIN}/haro-acp" in command
    # The real product wiring: acp spawns haro-agent, which gets haro-dev-mcp.
    assert env["HARO_ACP_AGENT_COMMAND"] == f"{REMOTE_BIN}/haro-agent"
    assert env["HARO_ACP_MCP_COMMAND"] == f"{REMOTE_BIN}/haro-dev-mcp"
    assert env["HARO_RELAY_URL"] == trial.relay_ws_url
    assert env["HARO_PRIVATE_KEY"] == orch.nostr_secret_key
    assert env["NOSTR_PRIVATE_KEY"] == orch.nostr_secret_key
    assert env["HARO_AGENT_NO_HINTS"] == "1"
    assert env["HARO_AGENT_MAX_ROUNDS"] == expected
    assert env["HARO_ACP_SYSTEM_PROMPT_FILE"].endswith("orch-1.system-prompt.md")
    # The composed prompt was uploaded into the container.
    assert any(
        target == env["HARO_ACP_SYSTEM_PROMPT_FILE"]
...
async def test_wait_for_agents_ready_requires_every_channel_subscription(tmp_path):
    ...
    from harbor_haro_orchestra.container_runtime import _Agent
...
async def test_wait_for_done_requires_orchestrator_authorship(tmp_path, monkeypatch):
    ...
    async def haro_json(credential, *args, **kwargs):
        observers.append(credential.agent_id)
        return next(rounds)

    monkeypatch.setattr(rt, "_haro_json", haro_json)
...
async def test_stop_agents_sweeps_the_uploaded_stack(tmp_path):
    ...
    await HaroContainerRuntime._stop_agents(environment, agents)
