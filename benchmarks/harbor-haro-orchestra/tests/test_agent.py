...
from harbor_haro_orchestra import (
    AgentCredential,
    HaroOrchestraAgent,
    RuntimeResult,
    TrialHandle,
...
async def test_missing_integrations_fail_explicitly(tmp_path, manifest_data):
    agent = HaroOrchestraAgent(logs_dir=tmp_path, manifest=manifest_data)
...
async def test_cli_runtime_construction_from_json(tmp_path, manifest_data):
    ...
    agent = HaroOrchestraAgent(
        ...
        haro_acp_binary="/pinned/haro-acp",
        haro_agent_binary="/pinned/haro-agent",
        haro_dev_mcp_binary="/pinned/haro-dev-mcp",
        haro_cli_binary="/pinned/haro",
    )
    assert agent.runtime.artifact_root == tmp_path
    assert agent.runtime.endpoints["frontier/rev"].provider == "anthropic"
    assert agent.runtime.haro_acp_binary == "/pinned/haro-acp"
    assert agent.runtime.haro_agent_binary == "/pinned/haro-agent"
    assert agent.runtime.haro_dev_mcp_binary == "/pinned/haro-dev-mcp"
    assert agent.runtime.haro_cli_binary == "/pinned/haro"
