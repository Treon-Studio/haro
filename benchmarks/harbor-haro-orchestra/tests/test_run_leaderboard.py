...
def test_command_uses_standard_settings_only(args, binaries, agent_binaries):
    ...
    assert any(k.startswith("haro_acp_binary=") for k in kwargs)
    assert any(k.startswith("haro_agent_binary=") for k in kwargs)
    assert any(k.startswith("haro_dev_mcp_binary=") for k in kwargs)


def test_agent_binaries_must_exist(tmp_path):
    with pytest.raises(SystemExit, match="haro-dev-mcp"):
