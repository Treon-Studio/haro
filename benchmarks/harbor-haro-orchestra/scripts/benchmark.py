#!/usr/bin/env python3
"""One-command benchmark: bring up the Haro stack in Docker and run it.
...
- A dedicated ``haro-benchmark`` compose project reusing the production
...
- ``--gui`` adds that user to the relay membership list and opens the Haro
...
Run inside the testbed environment (the just recipe does this):
    ...
    uv run --project benchmarks/harbor-haro-orchestra/testbed \
        benchmarks/harbor-haro-orchestra/scripts/benchmark.py [--gui] [...]
"""
...
COMPOSE_PROJECT = "haro-benchmark"
COMPOSE_FILES = (
...
GUI_BUNDLE_IDENTIFIER = "xyz.block.haro.app.benchmark"
...
AGENT_BINARIES = ("haro-acp", "haro-agent", "haro-dev-mcp")
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        help="Open the Haro desktop app as the benchmark user to watch the run live",
    )
    parser.add_argument(
...
def load_state() -> dict[str, str]:
    ...
    from harbor_haro_testbed.keys import generate_keypair, keypair_from_secret
...
def print_user_identity(state: dict[str, str]) -> None:
    ...
    nothing beyond it)."""
    from harbor_haro_testbed.keys import encode_nsec
...
def write_env_file(state: dict[str, str]) -> Path:
    ...
    lines = {
        "HARO_IMAGE": os.environ.get("HARO_IMAGE", "ghcr.io/block/haro:main"),
        "HARO_DOMAIN": "localhost",
        "RELAY_URL": f"ws://localhost:{RELAY_HTTP_PORT}",
        "HARO_MEDIA_BASE_URL": f"http://localhost:{RELAY_HTTP_PORT}/media",
        "HARO_MEDIA_SERVER_DOMAIN": "localhost",
        "HARO_CORS_ORIGINS": f"http://localhost:{RELAY_HTTP_PORT}",
        "HARO_REQUIRE_AUTH_TOKEN": "true",
        "HARO_REQUIRE_RELAY_MEMBERSHIP": "true",
        "HARO_ALLOW_NIP_OA_AUTH": "true",
        "HARO_AUTO_MIGRATE": "true",
        "HARO_GIT_CONFORMANCE_PROBE": "true",
        "RUST_LOG": "haro_relay=info,haro_db=info,haro_auth=info",
        "RELAY_OWNER_PUBKEY": state["owner_pubkey"],
        "HARO_RELAY_PRIVATE_KEY": state["relay_private_key"],
        "HARO_GIT_HOOK_HMAC_SECRET": state["git_hook_hmac_secret"],
"POSTGRES_DB": "haro",
"POSTGRES_USER": "haro",
        "POSTGRES_PASSWORD": state["postgres_password"],
        "REDIS_PASSWORD": state["redis_password"],
        "TYPESENSE_API_KEY": state["typesense_api_key"],
        "HARO_S3_ACCESS_KEY": state["s3_access_key"],
        "HARO_S3_SECRET_KEY": state["s3_secret_key"],
        "HARO_S3_BUCKET": "haro-media",
        "HARO_HTTP_PORT": str(RELAY_HTTP_PORT),
        "HARO_PG_HOST_PORT": str(PG_HOST_PORT),
        "HARO_METRICS_HOST_PORT": str(METRICS_HOST_PORT),
    }
...
def postgres_dsn(state: dict[str, str]) -> str:
    return (
f"postgresql://haro:{state['postgres_password']}"
f"@127.0.0.1:{PG_HOST_PORT}/haro"
    )
...
def ensure_binaries() -> dict[str, Path]:
    """Find the host haro CLI, building it once if missing."""
    try:
        return run_leaderboard.find_binaries(None)
    except SystemExit:
        print("host haro CLI missing — building (cargo build, first run only)...")
    cargo = REPO_ROOT / "bin" / "cargo"
    subprocess.run(
        [str(cargo), "build", "-p", "haro-cli"],
        cwd=REPO_ROOT,
        check=True,
    )
    return run_leaderboard.find_binaries(None)
...
def ensure_agent_binaries() -> Path:
    ...
    haro-acp → haro-agent → haro-dev-mcp stack, so the binaries must be
...
def launch_gui(state: dict[str, str]) -> subprocess.Popen:
    """Open the Haro desktop app logged in as the benchmark user.

    The relay runs closed (membership required), so the user pubkey is first
    added to the relay membership list via haro-admin inside the container —
    ...
    subprocess.run(
        compose_command(
            "exec", "-T", "relay",
            "haro-admin", "add-member", "--pubkey", state["user_pubkey"],
        ),
        check=True,
    )
    ...
    for name in ("haro-acp", "haro-agent", "haro-dev-mcp", "git-credential-nostr", "haro"):
        stub = sidecar_dir / f"{name}-{triple}"
        if not stub.exists():
            stub.touch()
    real_cli = sidecar_dir / f"haro-{triple}"
    real_cli.write_bytes(binaries["haro"].read_bytes())
    real_cli.chmod(0o755)

    print(
        f"Opening Haro GUI as the benchmark user ({state['user_pubkey'][:16]}…).\n"
    ...
    # workspace's relay URL overrides HARO_RELAY_URL by design. Reusing the
    ...
    tauri_config = json.dumps(
        {"identifier": GUI_BUNDLE_IDENTIFIER, "productName": "Haro Benchmark"}
    )
    return subprocess.Popen(
        ...
        env={
            **os.environ,
            "HARO_RELAY_URL": f"ws://localhost:{RELAY_HTTP_PORT}",
            "HARO_PRIVATE_KEY": state["user_secret_key"],
        },
    )
...
def leaderboard_argv(
    ...
    argv += [
        ...
        # HARO_BENCHMARK_DOCKER_HOST if your engine exposes the host
        # differently.
        "--relay-gateway",
        f"{os.environ.get('HARO_BENCHMARK_DOCKER_HOST', 'host.docker.internal')}"
        f":{RELAY_HTTP_PORT}",
