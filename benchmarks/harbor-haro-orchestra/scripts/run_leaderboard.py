...
importable:
    ...
    uv run --project benchmarks/harbor-haro-orchestra/testbed \
        ...
        --agent-bin-dir <DIR with Linux haro-acp/haro-agent/haro-dev-mcp>
"""
...
AGENT_IMPORT = "harbor_haro_orchestra:HaroOrchestraAgent"
PROVISIONER_FACTORY = "harbor_haro_testbed:provisioner_from_dict"
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        help="JSON config for the Haro relay/Postgres provisioner",
    )
    parser.add_argument(
        "--haro-bin-dir", type=Path, default=None,
        help="Directory with the host haro CLI (default: repo target/release, then target/debug)",
    )
    parser.add_argument(
        "--agent-bin-dir", type=Path, required=True,
        help="Directory with Linux builds of haro-acp/haro-agent/haro-dev-mcp "
...
def find_binaries(bin_dir: Path | None) -> dict[str, Path]:
    ...
    raise SystemExit(
        f"haro binaries not found (need {', '.join(BINARIES)}; searched {searched}). "
        "Build them with `cargo build` or pass --haro-bin-dir."
    )
...
def build_command(
    ...
    kwargs = {
        ...
        "haro_acp_binary": agent_binaries["haro-acp"],
        "haro_agent_binary": agent_binaries["haro-agent"],
        "haro_dev_mcp_binary": agent_binaries["haro-dev-mcp"],
        "haro_cli_binary": binaries["haro"],
...
def write_metadata_template(args: argparse.Namespace, job_dir: Path) -> Path:
    ...
    metadata = {
        "agent_url": "https://github.com/block/haro",
        "agent_display_name": f"Haro Orchestra ({manifest.get('condition', 'team')})",
...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.dry_run:
        ...
        bin_dir = args.haro_bin_dir or PACKAGE_ROOT.parents[1] / "target" / "release"
        ...
        return 0
    binaries = find_binaries(args.haro_bin_dir)
