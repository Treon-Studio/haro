"""Channel-per-trial Haro provisioning against a local benchmark stack."""
...
def provisioner_from_dict(config: dict[str, object]) -> "HaroTrialProvisioner":
    """Harbor CLI factory for a JSON-decoded testbed configuration."""
    return HaroTrialProvisioner(TestbedConfig(**config))


class HaroTrialProvisioner:
    """Implements the TrialHandle v1.1 contract against a live Haro relay.

    Guarantees (contract PLANS/HARBOR_HARO_TRIALHANDLE_CONTRACT.md):
    ...
    def _provision(
        ...
        # mirroring production Haro, where a human owns the channel their
        # agents work in.
    ...
    def _cli_for(self, credential: AgentCredential) -> HaroCli:
        return HaroCli(
