...
def test_env_file_wires_owner_and_ports(state_dir):
    ...
    assert env["HARO_HTTP_PORT"] == str(benchmark.RELAY_HTTP_PORT)
    assert env["HARO_PG_HOST_PORT"] == str(benchmark.PG_HOST_PORT)
    assert env["HARO_REQUIRE_RELAY_MEMBERSHIP"] == "true"


def test_compose_command_isolates_the_project(state_dir):
    command = benchmark.compose_command("up", "-d")
    assert command[:2] == ["docker", "compose"]
    assert command[command.index("--project-name") + 1] == "haro-benchmark"
