...
def test_mint_credentials_expands_roster(manifest):
    credentials = HaroTrialProvisioner(config())._mint_credentials(manifest)
...
def test_mint_credentials_keys_are_fresh_and_attested(manifest):
    provisioner = HaroTrialProvisioner(config())
...
def test_pinned_user_secret_reuses_one_identity():
    pinned = "7" * 64
    provisioner = HaroTrialProvisioner(config(user_secret_key=pinned))
...
def test_teardown_skips_archiving_when_disabled():
    provisioner = HaroTrialProvisioner(config(archive_on_teardown=False))
...
def test_mint_credentials_missing_api_key_is_explicit(manifest):
    provisioner = HaroTrialProvisioner(config(llm_api_keys={}))
...
def test_lock_key_is_deterministic_and_distinct():
    ...
    HaroTrialProvisioner._lock_trial(FakeConn(), "run-a", "trial-1")
    HaroTrialProvisioner._lock_trial(FakeConn(), "run-a", "trial-1")
    HaroTrialProvisioner._lock_trial(FakeConn(), "run-a", "trial-2")
...
def test_healthcheck_fails_fast_when_relay_down():
    provisioner = HaroTrialProvisioner(
