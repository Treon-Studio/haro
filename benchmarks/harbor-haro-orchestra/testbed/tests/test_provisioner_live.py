...
Gated by HARO_TESTBED_LIVE=1 with stack coordinates in the environment:
  HARO_TESTBED_RELAY_HTTP   (default http://localhost:3000)
  HARO_TESTBED_RELAY_WS     (default ws://host.docker.internal:3000)
  HARO_TESTBED_OWNER_KEY    relay owner secret key (hex)
  HARO_TESTBED_PG_DSN       benchmark Postgres DSN
"""
...
import psycopg
import pytest
from harbor_haro_testbed.haro_cli import HaroCli, HaroCliError
from harbor_haro_testbed.provisioner import (
...
pytestmark = pytest.mark.skipif(
    os.environ.get("HARO_TESTBED_LIVE") != "1",
    reason="live testbed suite; set HARO_TESTBED_LIVE=1 with a running stack",
)


@pytest.fixture()
def provisioner() -> HaroTrialProvisioner:
    owner_key = os.environ.get("HARO_TESTBED_OWNER_KEY")
    dsn = os.environ.get("HARO_TESTBED_PG_DSN")
    if not owner_key or not dsn:
        pytest.fail("HARO_TESTBED_OWNER_KEY and HARO_TESTBED_PG_DSN are required")
    return HaroTrialProvisioner(
        TestbedConfig(
            relay_http_url=os.environ.get(
                "HARO_TESTBED_RELAY_HTTP", "http://localhost:3000"
            ),
            relay_ws_url=os.environ.get(
                "HARO_TESTBED_RELAY_WS", "ws://host.docker.internal:3000"
            ),
...
def cli_for(provisioner: HaroTrialProvisioner, credential) -> HaroCli:
...
def test_create_is_idempotent_and_isolated(provisioner, manifest):
    ...
    try:
        ...
        with pytest.raises(HaroCliError, match="private"):
    ...
    provisioner.teardown(handle_a)
    with psycopg.connect(os.environ["HARO_TESTBED_PG_DSN"]) as conn:
        row = conn.execute(
