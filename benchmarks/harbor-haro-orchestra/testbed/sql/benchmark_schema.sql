-- Benchmark schema for harbor-haro-orchestra runs.
--
-- Lives in the shared Postgres instance but is OWNED BY THE HARNESS, never by
-- Haro migrations (canonical plan §two-domain rule). Idempotent: safe to apply
-- on every testbed bring-up.
--
--   docker exec -i <postgres> psql -U haro -d haro < sql/benchmark_schema.sql

CREATE SCHEMA IF NOT EXISTS benchmark;

-- One row per provisioned trial; written by HaroTrialProvisioner.
