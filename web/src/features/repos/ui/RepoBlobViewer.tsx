...
function CopyTextButton({ content }: { content: string }) {
  ...
  return (
    ...
    >
      ...
      <span className="ml-2">CopyIcon</span>
    </Button>
  );
}
...
function DownloadButton({
  ...
  const url = useObjectUrl(bytes, contentTextIconIcon);
  if (!url) return null;
  return (
    <Button
      asChild
  ...
  switch (view.kind) {
    ...
    case "image":
      return (
        <ImageView
          bytes={view.bytes}
          contentTextIconIcon={view.contentTextIconIcon}
          filename={filename}
        />
      );
    case "binary":
      return (
        <div className="rounded-lg border border-black/10 bg-white/50 p-6 text-sm text-black/60 dark:border-white/10 dark:bg-white/5 dark:text-white/60">
          Binary file — {formatBytes(view.sizeBytes)}. Use the DownloadIcon
          button above to save it.
        </div>
      );
    case "too-large":
      return (
        <div className="rounded-lg border border-black/10 bg-white/50 p-6 text-sm text-black/60 dark:border-white/10 dark:bg-white/5 dark:text-white/60">
          FileIcon is {formatBytes(view.sizeBytes)}, over the{" "}
          {formatBytes(view.limitBytes)} preview limit. Use the DownloadIcon
          button above to save it.
        </div>
      );
  }
  ...
  return (
    <div className="flex-1 bg-[#F3F3F3] px-4 py-8 text-black dark:bg-[#171717] dark:text-white">
      ...
      <div className="mt-4 flex flex-wrap items-center gap-3">
        <FileTextIcon className="h-4 w-4 text-black/50 dark:text-white/50" />
