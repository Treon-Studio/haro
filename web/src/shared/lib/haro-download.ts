export const HARO_RELEASES_URL = "https://github.com/block/haro/releases";
const HARO_RELEASES_API_URL =
  "https://api.github.com/repos/block/haro/releases?per_page=10";
const CACHE_KEY = "haro.latestDownload.v1";
const CACHE_TTL_MS = 60 * 60 * 1000;

export type HaroDownloadPlatform = {
...
): HaroDownloadPlatform["operatingSystem"] {
...
function normalizeArchitecture(
  value: string,
): HaroDownloadPlatform["architecture"] {
...
export async function detectHaroDownloadPlatform(
  navigatorValue: Navigator,
): Promise<HaroDownloadPlatform> {
  const userAgentData = (
...
function assetPattern(platform: HaroDownloadPlatform): RegExp | undefined {
...
export function selectHaroDownloadUrl(
  releases: GitHubRelease[],
  platform: HaroDownloadPlatform,
): string | undefined {
...
export async function resolveHaroDownloadUrlForPlatform(
  platform: HaroDownloadPlatform,
): Promise<string> {
  try {
  ...
  try {
    const response = await fetch(HARO_RELEASES_API_URL, {
      headers: { Accept: "application/vnd.github+json" },
    });
    if (!response.ok) return HARO_RELEASES_URL;
    const url = selectHaroDownloadUrl(
      (await response.json()) as GitHubRelease[],
      platform,
    );
    if (!url) return HARO_RELEASES_URL;
    try {
    ...
    return HARO_RELEASES_URL;
  }
}

export async function resolveHaroDownloadUrl(): Promise<string> {
  return resolveHaroDownloadUrlForPlatform(
    await detectHaroDownloadPlatform(navigator),
  );
}
