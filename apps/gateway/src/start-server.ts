...
if (
  ...
  // Admin routes for Virtual Keys, Configs, and Cache Invalidation
...
async function showLoadingAnimation() {
  ...
  return new Promise((resolve) => {
    const interval = setInterval(() => {
      process.stdout.write(`\r${frames[i]} Starting Haro Gateway...`);
      i = (i + 1) % frames.length;
    }, 80);
...
console.log('\x1b[1m%s\x1b[0m', '🚀 Haro Gateway is running at:');
