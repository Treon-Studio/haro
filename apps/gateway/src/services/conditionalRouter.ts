  ...
  private evaluateOperator(operator: string, value: any): boolean {
    for (const [op, compareValue] of Object.entries(operator)) {
      switch (op) {
        ...
        case Operator.Regex:
          try {
            let regex = regexCache.get(compareValue);
            if (!regex) {
              // Limit regex pattern length to prevent extremely complex/dangerous regex patterns
              if (compareValue.length > 500) {
                return false;
              }
              regex = new RegExp(compareValue);
              regexCache.set(compareValue, regex);
            }
