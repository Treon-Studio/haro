...
export { constructConfigFromRequestHeaders } from "./configConstructor";
import {
	ANTHROPIC,
	AZURE_AI_INFERENCE,
	AZURE_OPEN_AI,
...
import type { HookSpan } from "../middlewares/hooks";
...
export function selectProviderByWeight(providers: Options[]): Options {
	...
	const providersWithWeight = providers.map((provider) => ({
		...provider,
		weight: provider.weight ?? 1,
	}));

	// Compute the total weight
	const totalWeight = providersWithWeight.reduce(
		(sum: number, provider) => sum + provider.weight,
		0,
	);
	...
	for (const [index, provider] of providersWithWeight.entries()) {
