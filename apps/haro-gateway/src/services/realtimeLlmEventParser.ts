...
export class RealtimeLlmEventParser {
  ...
  private rebuildConversationSequence(items: Map<string, any>): string[] {
    ...
    // Map previous_item_id -> item data for O(1) lookups during traversal
    const prevToItemMap = new Map<string | null, any>();
    for (const val of itemValues) {
      prevToItemMap.set(val.previous_item_id, val);
    }

    // Find the first item (no previous_item_id)
    let currentId: string | undefined = prevToItemMap.get(null)?.item?.id;

    // Traverse through the chain using the map
    while (currentId) {
      orderedItemIds.push(currentId);
      const nextItem = prevToItemMap.get(currentId);
