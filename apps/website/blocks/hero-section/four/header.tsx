...
import { AltArrowRightLinear, HamburgerMenuLinear, CloseCircleLinear } from 'solar-icon-set';
import { Button } from '@/components/ui/button'
import React from 'react'
import { cn } from '@treonstudio/bungas-core/lib/utils'
import { motion, AnimatePresence, useScroll, useMotionValueEvent } from 'motion/react'
import { useMedia } from '@treonstudio/bungas-core/hooks/use-media'
                    ...
                    <div className="relative flex flex-wrap items-center justify-between gap-6 py-6 lg:gap-0">
                        <div className={cn('flex justify-between gap-6 duration-200 max-lg:w-full', isScrolled && 'lg:opacity-0 lg:blur-[4px]')}>
                            ...
                            <button
                                ...
                                aria-label={menuState == true ? 'Close HamburgerMenuLinear' : 'Open HamburgerMenuLinear'}
                                className="relative z-20 -m-2.5 -mr-4 block cursor-pointer p-2.5 lg:hidden">
                                <HamburgerMenuLinear className="in-data-[state=active]:rotate-180 in-data-[state=active]:scale-0 in-data-[state=active]:opacity-0 m-auto size-6 duration-200" />
                                <CloseCircleLinear className="in-data-[state=active]:rotate-0 in-data-[state=active]:scale-100 in-data-[state=active]:opacity-100 absolute inset-0 m-auto size-6 -rotate-180 scale-0 opacity-0 duration-200" />
                            </button>
                        </div>
