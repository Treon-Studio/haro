...
export default function Login() {
  ...
  const handleOAuth = useCallback(async (provider: "google" | "github") => {
    ...
    try {
      const res = await fetch(ROUTES.API.AUTH.OAUTH, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
      ...
      <div className="mx-auto flex w-full max-w-sm flex-col items-center justify-center gap-4">
        ...
        <Card className="w-full p-8">
          <form onSubmit={handleSubmit} className="space-y-5">
            ...
            <div className="flex items-center justify-between text-sm">
              <label className="flex items-center gap-2">
                <input type="checkbox" className="border-border rounded-sm" />
                <span className="text-muted-foreground">Remember me</span>
              </label>
              <a href={ROUTES.PAGE.FORGOT_PASSWORD} className="text-primary font-medium hover:underline">
                Forgot password?
              </a>
            </div>
          ...
          <div className="mt-6 text-center text-sm">
            ...
            <a href={ROUTES.PAGE.SIGNUP} className="text-primary font-medium hover:underline">
              Sign up
            </a>
          </div>
        </Card>
