...
helm lint deploy/charts/haro-push-gateway >/dev/null
helm template push deploy/charts/haro-push-gateway >"$out"
# Production values must attach push.haro.xyz to an explicit Gateway.
production_args=(
  -f deploy/charts/haro-push-gateway/values-production.yaml
  --set 'image.digest=sha256:aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa'
  --set 'appAttestAppId=REALTEAM.xyz.haro'
...
helm lint deploy/charts/haro-push-gateway "${production_args[@]}" >/dev/null
helm template push deploy/charts/haro-push-gateway "${production_args[@]}" >"$production_out"
...
runtime={'app.kubernetes.io/name':'haro-push-gateway','app.kubernetes.io/instance':'push','app.kubernetes.io/component':'runtime'}
...
assert jenv['haro_PUSH_RUNTIME_DATABASE_ROLE']['value']=='haro_push_gateway_runtime'
...
required={'DATABASE_URL','haro_PUSH_APNS_KEY_ID','haro_PUSH_APNS_TEAM_ID','haro_PUSH_APNS_TOPIC','haro_PUSH_GRANT_KEYS','haro_PUSH_TOKEN_KEYS','haro_PUSH_MAX_GRANT_LIFETIME_SECONDS'}
assert required <= env
...
np=next(x for x in nps if x['metadata']['name']=='push-haro-push-gateway')
migration_np=next(x for x in nps if x['metadata']['name']=='push-haro-push-gateway-migration')
...
assert 'push.haro.xyz' in route['spec']['hostnames']
PY

# Enabling a route without a Gateway attachment must fail schema validation.
if helm template push deploy/charts/haro-push-gateway --set httpRoute.enabled=true >/dev/null 2>&1; then
...
if helm template push deploy/charts/haro-push-gateway -f deploy/charts/haro-push-gateway/values-production.yaml >/dev/null 2>&1; then
...
helm template push deploy/charts/haro-push-gateway \
...
np=next(x for x in xs if x and x.get('kind')=='NetworkPolicy' and x['metadata']['name']=='push-haro-push-gateway')
...
if helm template push deploy/charts/haro-push-gateway \
  --set podMonitor.enabled=true \
