...
{{- $minimumReplicas := include "haro.minimumReplicas" . | int -}}
{{- if gt $minimumReplicas 1 -}}
  {{- if and (not .Values.redis.enabled) (not .Values.externalRedis.url) (not .Values.secrets.existingSecret) -}}
    {{- fail (printf "minimum replica count %d requires Redis for haro-pubsub. Enable redis.enabled=true, set externalRedis.url, or provide secrets.existingSecret with key REDIS_URL." $minimumReplicas) -}}
  {{- end -}}
{{- end -}}
...
{{/* Multiple replicas do NOT require ReadWriteMany git storage.
     ...
     multi-pod requirement for haro-pubsub. */}}
