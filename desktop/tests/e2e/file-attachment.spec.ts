...
async function attachQuarterlyReport(page: Page) {
...
test("upload a file and see a FileCard in the timeline", async ({ page }) => {
  ...
  // Paperclip → browser picker → mocked upload_media_bytes descriptor.
  await attachQuarterlyReport(page);
  ...
  const bufferedMessage = page.getByRole("button", { name: "1 new message" });
  await expect(bufferedMessage).toBeVisible();
  await bufferedMessage.click();
