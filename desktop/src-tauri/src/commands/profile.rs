...
pub async fn update_profile(
    ...
    let builder = build_merged_profile_event(
        &current,
        ProfileOverrides {
...
fn build_merged_profile_event(
    current: &Value,
    overrides: ProfileOverrides<'_>,
    prior_event: Option<&nostr::Event>,
) -> Result<nostr::EventBuilder, String> {
    let mut content = current.as_object().cloned().unwrap_or_default();
    for (key, value) in [
    ...
    Ok(
        nostr::EventBuilder::new(nostr::Kind::Metadata, content).custom_created_at(
...
mod tests {
    ...
    fn deferred_profile_event_is_strictly_newer_than_prior_head() {
        ...
        assert_eq!(content["lud16"], "larry@example.com");
        assert_eq!(content["extensions"]["theme"], "dark");
    }

    #[test]
    fn merged_profile_event_changes_only_defined_fields() {
        let keys = nostr::Keys::generate();
        let builder = build_merged_profile_event(
            &serde_json::json!({
        ...
        .expect("build merged profile");
        let event = builder.sign_with_keys(&keys).expect("sign merged profile");
        let content = serde_json::from_str::<Value>(&event.content).unwrap();

        assert_eq!(content["display_name"], "New name");
        assert_eq!(content["about"], "New bio");
        assert_eq!(content["name"], "larry");
        assert_eq!(content["nip05"], "larry@example.com");
        assert_eq!(content["website"], "https://example.com");
        assert_eq!(content["extensions"]["theme"], "dark");
    }
