...
use output::validate_uploaded_descriptor;
pub use output::{NativeMediaOutput, NativeMediaUploadResult};
...
async fn do_upload(
    ...
    let descriptor = parse_json_response::<BlobDescriptor>(resp).await?;
    validate_uploaded_descriptor(&descriptor, &output)?;
    Ok(NativeMediaUploadResult { descriptor, output })
}
...
pub async fn upload_media(
    ...
    Ok(do_upload(body, &mime, &state, None).await?.descriptor)
}
...
async fn process_picked_path(
    ...
    let mut descriptor = do_upload(body, &mime, state, None).await?.descriptor;

    if let Some(poster) = poster_bytes {
        match do_upload(poster, "image/jpeg", state, None).await {
            Ok(poster_upload) => descriptor.image = Some(poster_upload.descriptor.url),
...
pub async fn upload_media_bytes(
    ...
    let mut upload = do_upload(body, &mime, &state, progress).await?;

    if let Some(poster) = poster_bytes {
        match do_upload(poster, "image/jpeg", &state, None).await {
            Ok(poster_upload) => {
                upload.descriptor.image = Some(poster_upload.descriptor.url);
            }
            Err(e) => eprintln!("buzz-desktop: poster upload failed (non-fatal): {e}"),
        }
    }

    upload.descriptor.filename = filename.as_deref().map(sanitize_filename);
