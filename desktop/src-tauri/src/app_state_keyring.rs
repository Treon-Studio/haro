...
fn dev_keyring_service(configured: Option<String>) -> String {
    configured
        .filter(|service| {
            service.starts_with("haro-desktop-dev.") || service.starts_with("buzz-desktop-dev.")
        })
        .unwrap_or_else(|| "haro-desktop-dev".to_string())
}
...
pub(crate) fn keyring_service() -> &'static str {
    if cfg!(debug_assertions) {
        ...
        DEV_SERVICE
            .get_or_init(|| {
                dev_keyring_service(
                    std::env::var("HARO_DEV_KEYRING_SERVICE")
                        .or_else(|_| std::env::var("BUZZ_DEV_KEYRING_SERVICE"))
                        .ok(),
                )
            })
            .as_str()
...
pub(super) fn migration_marker_name(service: &str, default_name: &str) -> String {
    if service == "haro-desktop"
        || service == "haro-desktop-dev"
