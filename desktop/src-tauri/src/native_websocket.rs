        ...
        tokio::select! {
            ...
            incoming = socket.next() => {
                ...
                if let Ok(value) = serde_json::to_value(message) {
                    // A removed renderer callback means the owning session was
                    // torn down. Treat the late native completion as cancelled
                    // and stop this task instead of retaining an orphan socket.
...
mod tests {
    ...
    fn cancelled_channel() -> Channel<serde_json::Value> {
        Channel::new(|_: InvokeResponseBody| {
            Err(std::io::Error::other("callback registration removed").into())
        })
    }
    ...
    #[tokio::test]
    async fn removed_callback_cancels_late_message_delivery() {
        let manager = WebSocketManager::default();
        let (client_io, server_io) = duplex(1024);
        ...
        manager.connections.lock().await.insert(2, handle.clone());
        let task = tauri::async_runtime::spawn(run_connection(
            2,
            client,
            receiver,
        ...
        *handle.task.lock().await = Some(task);
        ...
        server
            .send(Message::Text("late completion".into()))
            .await
            .unwrap();
        tokio::time::timeout(Duration::from_secs(1), async {
            while manager.connections.lock().await.contains_key(&2) {
                tokio::task::yield_now().await;
            }
        })
        .await
        .expect("a removed callback should cancel native delivery");
    }
