 ...
 * HaroLogoAnimation morph (v8 variant), rendered in `currentColor`, so it
 * paints complete on the very first frame regardless of animation support.
 */
export function HaroMark({ className }: { className?: string }) {
  const maskId = `haro-mark-cutouts-${useId().replace(/[^a-zA-Z0-9_-]/g, "")}`;

  return (
    <svg
      aria-hidden="true"
      className={["haro-mark", className].filter(Boolean).join(" ")}
      viewBox="0 0 466 309"
      fill="currentColor"
    >
      <defs>
