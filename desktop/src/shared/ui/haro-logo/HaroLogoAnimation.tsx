...
import "./haro-logo-animation.css";
...
export type HaroLogoAnimationProps = {
  ariaLabel?: string;
  className?: string;
  fullScreen?: boolean;
  loop?: boolean;
...
export default function HaroLogoAnimation({
  ...
  const maskId = `haro-logo-cutouts-${idSuffix}`;
  const textureId = `haro-logo-texture-${idSuffix}`;
  const texture = TEXTURES[config.texture ?? "soft"] ?? TEXTURES.soft;
  const classes = [
    "haro-logo",
    fullScreen && "haro-logo--screen",
    !fullScreen && "haro-logo--compact",
    showBackground && "haro-logo--background",
    className,
  ]
    .filter(Boolean)
    .join(" ");
