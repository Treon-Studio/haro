...
export const KIND_LEGACY_STREAM_MESSAGE = 40001;
...
export const KIND_GROUP_MEMBER_LIST = 9000;
...
export const CHANNEL_EVENT_KINDS = [
  ...
  KIND_LEGACY_STREAM_MESSAGE, // legacy: pre-migration stream messages
...
// Full live channel surface. Every kind here is scoped by an `h` tag and is
// routed through the typed realtime mapper. Keep this as the single source for
// RelayClient-backed channel subscriptions.
export const CHANNEL_LIVE_EVENT_KINDS = [
  ...CHANNEL_EVENT_KINDS,
...
// Auxiliary channel events may arrive without an `h` tag when the relay
// provides their channel through the subscription context. They are accepted
// only when they carry an `e` reference to the message they modify.
export const CHANNEL_CONTEXTUAL_EVENT_KINDS = [
  KIND_DELETION,
  KIND_REACTION,
...
// Community-wide typed realtime families. These are subscribed lazily when a
// corresponding SimpleSocket handler is installed.
export const GLOBAL_REALTIME_EVENT_KINDS = [
  KIND_DELETION,
  KIND_PROFILE,
