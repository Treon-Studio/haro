...
test("cancel clears reconnect timers before their callbacks can run", async () => {
  const ctrl = new RelayReconnectController();
  const deps = makeDeps({
    preconnectResult: async () => {
      throw new Error("relay unreachable");
  ...
  await ctrl.start(deps);
  const pollId = deps._intervals[0]?.id;
  const backstopId = deps._timers[0]?.id;
  assert.ok(pollId !== undefined, "poll interval scheduled");
  assert.ok(backstopId !== undefined, "backstop timer scheduled");

  ctrl.cancel();
  await deps._fireInterval(pollId);
  deps._fireTimer(backstopId);

  assert.equal(deps.preconnect.mock.calls.length, 1, "no late poll reconnect");
  assert.equal(
    deps.onBackstop.mock.calls.length,
    0,
    "no late backstop callback",
  );
  assert.equal(deps._intervals.length, 0, "poll interval removed");
  assert.equal(deps._timers.length, 0, "backstop timer removed");
});
