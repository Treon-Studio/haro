...
import {
  ...
  createLiveSubscriptionReadiness,
  createTauriCallbackRegistration,
  HISTORY_TIMEOUT_MS,
  PUBLISH_TIMEOUT_MS,
  type TauriCallbackRegistration,
} from "@/shared/api/relayClientLifecycle";
...
/** Allow two 30-second relay heartbeat windows before treating the socket as stalled. */
const STALL_CHECK_INTERVAL_MS = 10_000,
  STALL_IDLE_TIMEOUT_MS = 60_000;
...
export class RelayClient {
  ...
  private onMessageRegistration: TauriCallbackRegistration<unknown> | null =
    null;
   ...
   *
   * Cleared only on explicit user re-engagement through `preconnect()`.
   */
  ...
  disconnect() {
    ...
    for (const [subId, sub] of this.subscriptions) {
      if (sub.mode === "history") {
        ...
        if (sub.readyTimeout !== undefined) {
          window.clearTimeout(sub.readyTimeout);
        }
        sub.rejectReady?.(error);
        sub.resolveReady = undefined;
        sub.rejectReady = undefined;
        sub.readyTimeout = undefined;
  ...
  private async connect() {
    ...
    this.unregisterMessageChannel();
    this.onMessageRegistration = createTauriCallbackRegistration<unknown>(
      (message) => {
    ...
    this.onMessageChannel = this.onMessageRegistration.channel;
  ...
  private async subscribe(
    ...
    const readiness = createLiveSubscriptionReadiness();
    ...
    this.subscriptions.set(subId, {
      ...
      rejectReady: readiness.rejectReady,
  ...
  async publishEvent(
    event: RelayEvent,
    timeoutMessage = "Failed to publish event: timeout",
    sendErrorMessage = "Failed to send event frame",
  ) {
  ...
  private unregisterMessageChannel() {
    this.onMessageRegistration?.unregister();
    this.onMessageRegistration = null;
  ...
  private resetConnection(
    ...
    for (const [subId, subscription] of this.subscriptions) {
      ...
      subscription.readyTimeout = undefined;
