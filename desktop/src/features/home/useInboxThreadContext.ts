...
export function buildInboxThreadDescendantFilter(
  channelId: string,
  threadRootId: string,
  ...
  React.useEffect(() => {
    ...
    async function loadContext() {
      ...
      try {
        ...
        const descendantEventsPromise = selectedChannelId
          ? realtimeSocketClient
              .fetchEvents(
                buildInboxThreadDescendantFilter(
                  selectedChannelId,
                  threadRootId,
                ),
              )
  ...
  > => {
    ...
    try {
      return await realtimeSocketClient.fetchEvents(
        buildChannelReactionAuxFilter(selectedChannelId, eventIds),
      );
    } catch (error) {
      console.error(
