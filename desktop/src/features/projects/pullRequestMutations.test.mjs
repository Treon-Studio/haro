...
test("pull request creation signs and publishes a NIP-34 event", async () => {
  let published = null;
  const id = await publishProjectPullRequest(
    project,
    {
      title: "Relay adapter",
      body: "Use events",
  ...
  assert.equal(published.kind, 1618);
  assert.equal(published.content, "Use events");
  assert.ok(published.tags.some((tag) => tag[0] === "a"));
  assert.equal(typeof published.sig, "string");
});

test("pull request creation throws when the relay rejects its signed event", async () => {
