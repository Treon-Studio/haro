...
async function updateProjectPullRequestStatus({
  ...
  const result1 = await simpleSocket.publishEvent(event);
  if (
    result1 &&
    ...
    throw new Error(result1.message || "Relay rejected status update.");
  }
}
...
async function requestProjectPullRequestReview({
  ...
  const result2 = await simpleSocket.publishEvent(event);
  if (
    result2 &&
    ...
    throw new Error(result2.message || "Relay rejected review request.");
  }
}
...
async function submitProjectPullRequestReview({
  ...
  const result3 = await simpleSocket.publishEvent(event);
  if (
    result3 &&
    ...
    throw new Error(result3.message || details.errorMessage);
  }
}
