...
import { getPresence, signRelayEvent } from "@/shared/api/tauri";
...
/** Sign and publish one ephemeral Buzz presence update. */
export async function publishPresenceUpdate(
  status: PresenceStatus,
  publisher: Pick<typeof simpleSocket, "publishEvent"> = simpleSocket,
  signer: typeof signRelayEvent = signRelayEvent,
) {
  ...
  if ("accepted" in result && result.accepted === false) {
    throw new Error(result.message || "Relay rejected presence update.");
  }
  return {
    status,
...
export function usePresenceQuery(
  ...
  const connected = true; // migrated away from relayClient
...
export function usePresenceSubscription() {
  ...
  React.useEffect(() => {
    ...
    function handlePresenceEvent({ event }: { event: RelayEvent }) {
    ...
    const unsubscribePresence = simpleSocket.on(
      "presence_update",
      handlePresenceEvent,
    );

    const unsubReconnect =
      simpleSocket.subscribeToReconnects?.(() => {
        if (!isCancelled)
...
export function useSetPresenceMutation(pubkey?: string) {
  ...
  return useMutation({
    mutationFn: (status: PresenceStatus) => publishPresenceUpdate(status),
    onSuccess: ({ status }) => {
...
export function usePresenceSession(pubkey?: string) {
  ...
  React.useEffect(() => {
    ...
    const intervalId = window.setInterval(() => {
      ...
      if (
        simpleSocket.getConnectionState?.() !== "connected" ||
        isRateLimited()
      ) {
        return;
      }
