...
export function useChannelSections(
  ...
  const applyRemote = React.useCallback(
    (
      ...
      return (prev) => {
        ...
        if (remote.updatedAt < lastAppliedRemoteTs.current) return prev;
        lastAppliedRemoteTs.current = remote.updatedAt;
  ...
  React.useEffect(() => {
    ...
    const unsubscribe = simpleSocket.on("connect", handler);
    return () => {
      cancelled = true;
      unsubscribe();
    };
