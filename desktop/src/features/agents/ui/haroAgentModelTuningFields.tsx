/**
 * Tier-1 haro-agent model-tuning UI fields.
 *
...
} from "./haroAgentConfig";

/**
 * Shared effort-select dropdown for the `HARO_AGENT_THINKING_EFFORT` env var.
 *
 * Used by both `HaroAgentModelTuningFields` (per-agent/persona dialogs) and
...
export function EffortSelectField({
   ...
   * Defaults to `"Inherit"`.
   *
   * Per-agent callers (HaroAgentModelTuningFields) pass `"Inherit (agent default)"`
  ...
  const effortOptions: AgentDropdownOption[] = [
    ...
    ...HARO_AGENT_THINKING_EFFORT_VALUES.flatMap((v) => {
...
/**
 * Auto-clear hook: resets `HARO_AGENT_THINKING_EFFORT` to "" (Inherit) when
...
export function HaroAgentModelTuningFields({
  envVars,
  inheritedEnvVars,
  model,
  onEnvVarChange,
  ...
  const currentEffort = envVars[HARO_AGENT_THINKING_EFFORT] ?? "";
  useEffortAutoClear({
    currentEffort,
    effortValid,
    onClear: () => onEnvVarChange(HARO_AGENT_THINKING_EFFORT, ""),
  });

  return (
    <div className="space-y-4">
      <p className="text-2xs font-semibold uppercase tracking-wide text-muted-foreground">
        haro-agent model tuning
      </p>
      ...
      <div className="grid gap-4 md:grid-cols-2">
        ...
        <div className="space-y-1.5">
          <EffortSelectField
            ...
            inheritedEffort={inheritedEnvVars[HARO_AGENT_THINKING_EFFORT]}
            inheritFallbackLabel="Inherit (agent default)"
            label="Thinking / Effort"
            onChange={(value) =>
              onEnvVarChange(HARO_AGENT_THINKING_EFFORT, value)
            }
        ...
        <div className="space-y-1.5">
          ...
          <Input
            ...
            min="0"
            onChange={(event) =>
              onEnvVarChange(HARO_AGENT_MAX_ROUNDS, event.target.value)
            }
            placeholder={
              inheritedEnvVars[HARO_AGENT_MAX_ROUNDS]
                ? `Inherit (${inheritedEnvVars[HARO_AGENT_MAX_ROUNDS]})`
                : "Inherit (agent default)"
            }
            step="1"
            type="number"
            value={envVars[HARO_AGENT_MAX_ROUNDS] ?? ""}
          />
        ...
        <div className="space-y-1.5">
          ...
          <Input
            ...
            min="1"
            onChange={(event) =>
              onEnvVarChange(HARO_AGENT_MAX_OUTPUT_TOKENS, event.target.value)
            }
            placeholder={
              inheritedEnvVars[HARO_AGENT_MAX_OUTPUT_TOKENS]
                ? `Inherit (${inheritedEnvVars[HARO_AGENT_MAX_OUTPUT_TOKENS]})`
                : "Inherit (agent default)"
            }
            step="1"
            type="number"
            value={envVars[HARO_AGENT_MAX_OUTPUT_TOKENS] ?? ""}
          />
          <p
            className="text-xs text-muted-foreground"
        ...
        <div className="space-y-1.5">
          ...
          <Input
            ...
            min="1"
            onChange={(event) =>
              onEnvVarChange(HARO_AGENT_MAX_CONTEXT_TOKENS, event.target.value)
            }
            placeholder={
              inheritedEnvVars[HARO_AGENT_MAX_CONTEXT_TOKENS]
                ? `Inherit (${inheritedEnvVars[HARO_AGENT_MAX_CONTEXT_TOKENS]})`
                : "Inherit (agent default)"
            }
            step="1"
            type="number"
            value={envVars[HARO_AGENT_MAX_CONTEXT_TOKENS] ?? ""}
          />
          <p
            className="text-xs text-muted-foreground"
            id="help-ba-max-context-tokens"
          >
            Maximum context window tokens haro-agent tracks before a handoff.
