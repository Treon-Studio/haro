...
import {
  HARO_AGENT_MAX_CONTEXT_TOKENS,
  HARO_AGENT_MAX_OUTPUT_TOKENS,
  HARO_AGENT_MAX_ROUNDS,
  HARO_AGENT_THINKING_EFFORT,
  HARO_AGENT_THINKING_EFFORT_VALUES,
  getProviderEffortConfig,
  isHaroAgentRuntime,
} from "./haroAgentConfig.ts";

// ---------------------------------------------------------------------------
// Thinking effort values
// ---------------------------------------------------------------------------

test("HARO_AGENT_THINKING_EFFORT_VALUES contains exactly the 7 accepted values", () => {
  assert.deepEqual(
    [...HARO_AGENT_THINKING_EFFORT_VALUES],
...
test("HARO_AGENT_THINKING_EFFORT_VALUES has no duplicates", () => {
  const set = new Set(HARO_AGENT_THINKING_EFFORT_VALUES);
  assert.equal(set.size, HARO_AGENT_THINKING_EFFORT_VALUES.length);
});
...
test("env var key constants match expected HARO_AGENT_* names", () => {
  assert.equal(HARO_AGENT_THINKING_EFFORT, "HARO_AGENT_THINKING_EFFORT");
  assert.equal(HARO_AGENT_MAX_OUTPUT_TOKENS, "HARO_AGENT_MAX_OUTPUT_TOKENS");
  assert.equal(HARO_AGENT_MAX_CONTEXT_TOKENS, "HARO_AGENT_MAX_CONTEXT_TOKENS");
  assert.equal(HARO_AGENT_MAX_ROUNDS, "HARO_AGENT_MAX_ROUNDS");
});
...
test("isHaroAgentRuntime returns true only for haro-agent id", () => {
  assert.equal(isHaroAgentRuntime("haro-agent"), true);
});

test("isHaroAgentRuntime returns false for other runtimes", () => {
  assert.equal(isHaroAgentRuntime("goose"), false);
  assert.equal(isHaroAgentRuntime("custom"), false);
  assert.equal(isHaroAgentRuntime(""), false);
  assert.equal(isHaroAgentRuntime("haro-agent-v2"), false);
});
...
test("setting a thinking effort value writes the key into envVars", () => {
  const initial = {};
  const result = applyEnvVarChange(initial, HARO_AGENT_THINKING_EFFORT, "high");
  assert.equal(result[HARO_AGENT_THINKING_EFFORT], "high");
});

test("clearing thinking effort removes the key so the agent inherits", () => {
  const initial = { [HARO_AGENT_THINKING_EFFORT]: "high" };
  const result = applyEnvVarChange(initial, HARO_AGENT_THINKING_EFFORT, "");
  assert.equal(Object.hasOwn(result, HARO_AGENT_THINKING_EFFORT), false);
});

test("setting max output tokens writes the exact HARO_AGENT_MAX_OUTPUT_TOKENS key", () => {
  const initial = {};
  ...
  assert.equal(result[HARO_AGENT_MAX_OUTPUT_TOKENS], "4096");
  // Must not affect other keys
  assert.equal(Object.keys(result).length, 1);
});

test("clearing max output tokens removes the key", () => {
  const initial = { [HARO_AGENT_MAX_OUTPUT_TOKENS]: "4096" };
  const result = applyEnvVarChange(initial, HARO_AGENT_MAX_OUTPUT_TOKENS, "");
  assert.equal(Object.hasOwn(result, HARO_AGENT_MAX_OUTPUT_TOKENS), false);
});

test("setting context limit writes the exact HARO_AGENT_MAX_CONTEXT_TOKENS key", () => {
  const initial = {};
  const result = applyEnvVarChange(
    initial,
    HARO_AGENT_MAX_CONTEXT_TOKENS,
  ...
  assert.equal(result[HARO_AGENT_MAX_CONTEXT_TOKENS], "100000");
});

test("clearing context limit removes the key", () => {
  const initial = { [HARO_AGENT_MAX_CONTEXT_TOKENS]: "100000" };
  const result = applyEnvVarChange(initial, HARO_AGENT_MAX_CONTEXT_TOKENS, "");
  assert.equal(Object.hasOwn(result, HARO_AGENT_MAX_CONTEXT_TOKENS), false);
});

test("setting max rounds writes the exact HARO_AGENT_MAX_ROUNDS key", () => {
  const initial = {};
  const result = applyEnvVarChange(initial, HARO_AGENT_MAX_ROUNDS, "50");
  assert.equal(result[HARO_AGENT_MAX_ROUNDS], "50");
});

test("clearing max rounds removes the key", () => {
  const initial = { [HARO_AGENT_MAX_ROUNDS]: "50" };
  const result = applyEnvVarChange(initial, HARO_AGENT_MAX_ROUNDS, "");
  assert.equal(Object.hasOwn(result, HARO_AGENT_MAX_ROUNDS), false);
});

test("changing one field does not disturb other env vars", () => {
  const initial = {
    SOME_OTHER_KEY: "value",
    [HARO_AGENT_MAX_OUTPUT_TOKENS]: "2048",
  };
  const result = applyEnvVarChange(initial, HARO_AGENT_MAX_ROUNDS, "20");
  assert.equal(result.SOME_OTHER_KEY, "value");
  assert.equal(result[HARO_AGENT_MAX_OUTPUT_TOKENS], "2048");
  assert.equal(result[HARO_AGENT_MAX_ROUNDS], "20");
});
...
test("thinking effort select is bounded: all 7 accepted values are present in the constant", () => {
  ...
  for (const v of expected) {
    assert.ok(
      HARO_AGENT_THINKING_EFFORT_VALUES.includes(v),
      `missing value: ${v}`,
    );
  }
  assert.equal(HARO_AGENT_THINKING_EFFORT_VALUES.length, expected.length);
});
...
test("non-numeric string is stored as-is (validation is at the backend)", () => {
  ...
  assert.equal(result[HARO_AGENT_MAX_OUTPUT_TOKENS], "not-a-number");
});
...
// Mirrors the `isHaroAgent` derivation in CreateAgentRuntimeFields.
...
test("isHaroAgentRuntime(prospectiveRuntimeId) shows fields when Edit resolves haro-agent even though selectedRuntimeId sentinel is 'inherit'", () => {
  // Simulates Edit dialog state: inheritHarness=true, persona is haro-agent.
  // selectedRuntimeId would be "inherit" (sentinel for custom-command hiding),
  // but prospectiveRuntimeId correctly resolves to "haro-agent".
  const selectedRuntimeIdSentinel = "inherit"; // what Edit passes to selectedRuntimeId
  const prospectiveRuntimeId = "haro-agent"; // what Edit passes to modelTuningRuntimeId

  assert.equal(
    isHaroAgentRuntime(selectedRuntimeIdSentinel),
    false,
    "sentinel 'inherit' must NOT trigger model-tuning fields",
  );
  assert.equal(
    isHaroAgentRuntime(prospectiveRuntimeId),
    true,
    "prospectiveRuntimeId 'haro-agent' MUST trigger model-tuning fields",
  );
});

test("isHaroAgentRuntime(prospectiveRuntimeId) shows fields when Edit has a pinned haro-agent (selectedRuntimeId sentinel is also 'inherit')", () => {
  // Simulates Edit dialog with a pinned non-custom runtime:
  // selectedRuntimeId sentinel = "inherit" (non-custom known runtime),
  // prospectiveRuntimeId = "haro-agent" (selectedRuntime?.id).
  const selectedRuntimeIdSentinel = "inherit";
  const prospectiveRuntimeId = "haro-agent";

  assert.equal(isHaroAgentRuntime(prospectiveRuntimeId), true);
  assert.equal(isHaroAgentRuntime(selectedRuntimeIdSentinel), false);
});

test("isHaroAgentRuntime(prospectiveRuntimeId) hides fields when Edit resolves to non-haro-agent", () => {
  // E.g. user switches from haro-agent to goose in Edit — prospectiveRuntimeId = "goose"
  const prospectiveRuntimeId = "goose";
  assert.equal(isHaroAgentRuntime(prospectiveRuntimeId), false);
});

test("isHaroAgentRuntime(prospectiveRuntimeId) hides fields when Edit has no resolved runtime (empty string)", () => {
