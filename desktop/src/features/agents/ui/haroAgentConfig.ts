/**
 * Source-of-truth constants for haro-agent model-tuning configuration knobs.
 *
 * Values must stay in sync with `crates/haro-agent/src/config.rs`
 * `parse_thinking_effort` — that function is the authoritative list.
 */

/** Env var key for the thinking/effort level sent to the LLM. */
export const HARO_AGENT_THINKING_EFFORT = "HARO_AGENT_THINKING_EFFORT";

/** Env var key for the maximum output token count per turn. */
export const HARO_AGENT_MAX_OUTPUT_TOKENS = "HARO_AGENT_MAX_OUTPUT_TOKENS";

/** Env var key for the context window token limit. */
export const HARO_AGENT_MAX_CONTEXT_TOKENS = "HARO_AGENT_MAX_CONTEXT_TOKENS";

/** Env var key for the maximum number of LLM/tool rounds per turn. */
export const HARO_AGENT_MAX_ROUNDS = "HARO_AGENT_MAX_ROUNDS";

/**
 * Ordered set of valid thinking-effort values accepted by haro-agent.
 * Mirrors `parse_thinking_effort` in `crates/haro-agent/src/config.rs`.
 */
export const HARO_AGENT_THINKING_EFFORT_VALUES = [
  "none",
  "minimal",
  "low",
  "medium",
...
export type ThinkingEffortValue =
  (typeof HARO_AGENT_THINKING_EFFORT_VALUES)[number];
 ...
 *
 * Mirrors the model-family tables in `crates/haro-agent/src/config.rs`
...
const ALL_VALUES = HARO_AGENT_THINKING_EFFORT_VALUES;
...
/**
 * Returns true when the given runtime id is haro-agent, which is the only
 * runtime that supports the tier-1 model-tuning knobs above.
 */
export function isHaroAgentRuntime(runtimeId: string): boolean {
  return runtimeId === "haro-agent";
}
