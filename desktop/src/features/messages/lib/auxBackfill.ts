...
import { realtimeSocketClient } from "@/shared/api/realtimeSocketClient";
import {
...
const defaultStructuralAuxDeps: StructuralAuxFetchDeps = {
  fetchAuxEventsForMessages: async (channelId, messageIds) => {
    try {
      return await realtimeSocketClient.fetchEvents(
        buildChannelStructuralAuxFilter(channelId, messageIds),
      );
    } catch {
      return [];
    }
  },
  fetchAuxDeletionEventsForAuxEvents: async (channelId, auxEventIds) => {
    try {
      return await realtimeSocketClient.fetchEvents(
        buildChannelAuxDeletionFilter(channelId, auxEventIds),
      );
    } catch {
      return [];
    }
...
export async function backfillAuxForMessages(
  ...
  try {
    ...
    try {
      auxEvents = await realtimeSocketClient.fetchEvents(
        buildChannelStructuralAuxFilter(channelId, messageIds),
      );
    } catch {}
    ...
    const mergedAuxEvents = await mergeAuxEventsWithDeletionBackfill({
      ...
      cachedEvents,
      fetchedAuxEvents: auxEvents,
      fetchAuxEventsForMessages: async (auxChannelId, auxEventIds) => {
        try {
          return await realtimeSocketClient.fetchEvents(
            buildChannelAuxDeletionFilter(auxChannelId, auxEventIds),
          );
        } catch {
          return [];
        }
