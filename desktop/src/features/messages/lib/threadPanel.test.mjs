...
test("mergeThreadPanelEntries keeps authoritative metadata and live fallbacks", () => {
  const richReply = {
    message: message({ id: "reply", reactions: [{ emoji: "👍" }] }),
    summary: null,
  };
  const bareReply = {
  ...
  const liveOnly = {
    message: message({ id: "live-only" }),
    summary: null,
  };

  assert.deepEqual(
    mergeThreadPanelEntries([richReply], [bareReply, liveOnly]),
    [richReply, liveOnly],
  );
});
