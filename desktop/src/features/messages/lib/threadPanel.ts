...
export function mergeThreadPanelEntries(
  authoritative: MainTimelineEntry[],
  fallback: MainTimelineEntry[],
): MainTimelineEntry[] {
  if (fallback.length === 0) return authoritative;
  const authoritativeIds = new Set(
    authoritative.map((entry) => entry.message.id),
  );
  return [
    ...authoritative,
    ...fallback.filter((entry) => !authoritativeIds.has(entry.message.id)),
  ];
}
