...
import { fetchChannelMessages } from "@/shared/api/chatStore";
...
function toLiveThreadMessage(event: RelayEvent): TimelineMessage {
...
export function MessageThreadPanel({
  ...
  const [internalReplies, setInternalReplies] = React.useState<
    MainTimelineEntry[]
  >([]);
  React.useEffect(() => {
    if (!channelId || !threadHead) return;
    let mounted = true;
    fetchChannelMessages(channelId)
      .then((msgs) => {
        if (mounted) {
          // Just filter for messages belonging to this thread and map to MainTimelineEntry
          const replies = msgs
            .filter((message) => {
              const thread = getThreadReference(message.tags ?? []);
              return (
                thread.rootId === threadHead.id ||
                thread.parentId === threadHead.id
              );
            })
            .map((message) => {
              const thread = getThreadReference(message.tags ?? []);
          ...
          setInternalReplies(replies);
        }
      })
      .catch(console.error);

    const unsubscribe = simpleSocket.on(
      "message_send",
      ({ event, channelId: incomingChannelId }) => {
        const thread = getThreadReference(event.tags);
        if (
          incomingChannelId === channelId &&
          (thread.rootId === threadHead.id || thread.parentId === threadHead.id)
        ) {
          setInternalReplies((prev) => {
            if (prev.find((message) => message.message.id === event.id))
              return prev;
            return [
              ...prev,
              { message: toLiveThreadMessage(event), summary: null },
            ];
          });
        }
      },
  ...
  }, [channelId, threadHead?.id, threadHead]);

  const threadRepliesToUse = React.useMemo(() => {
    return mergeThreadPanelEntries(threadReplies, internalReplies);
  }, [internalReplies, threadReplies]);

  const deferredThreadReplies = React.useDeferredValue(
    threadRepliesToUse,
    EMPTY_THREAD_REPLIES,
  );
  const isRepliesPending = deferredThreadReplies !== threadRepliesToUse;
