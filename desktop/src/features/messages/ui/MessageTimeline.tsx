...
function toLiveTimelineMessage(event: RelayEvent): TimelineMessage {
  return {
    id: event.id,
...
>(function MessageTimeline(
      ...
      ({ event, channelId: incomingChannelId }) => {
        if (incomingChannelId === channelId) {
          setInternalMessages((prev) => {
            if (prev.find((message) => message.id === event.id)) return prev;
            return [...prev, toLiveTimelineMessage(event)];
          });
        }
      },
    );
  ...
  const messagesToUse =
    internalMessages.length > 0 ? internalMessages : messages;
  ...
  const liveSnapshot = React.useMemo<TimelineSnapshot>(
    ...
    [channelId, historyExhausted, messagesToUse],
  );
  ...
  const timelineBodySurface = selectTimelineBodySurface({
    ...
    liveCount: messagesToUse.length,
  });
  ...
  const timelineIntroSurface = selectTimelineIntroSurface({
    ...
    hasReachedChannelStart:
      ...
      !isHoldingPrepend &&
      (messagesToUse.length === 0 || (!hasOlderMessages && !isFetchingOlder)),
  ...
  const timelineList = showMessageList ? (
    <TimelineMessageList
      ...
      mainEntries={renderedMessages === messagesToUse ? mainEntries : undefined}
