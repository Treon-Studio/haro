...
/** Throws the relay's rejection reason before callers synthesize an optimistic event. */
export function requireAcceptedMessageSend(
  result: RelayEvent | PublishResult,
): void {
  if ("accepted" in result && result.accepted === false) {
    throw new Error(result.message || "Relay rejected message.");
  }
}
...
export function useChannelSubscription(channel: Channel | null) {
  ...
  useEffect(() => {
    ...
    const disposeReconnectListener = simpleSocket.subscribeToReconnects(() => {
...
export function useSendMessageMutation(
  ...
  >({
    mutationFn: async ({
      ...
      const sendResult = await simpleSocket.sendMessage(
        effectiveChannel.id,
        content,
        recipientPubkeys,
        mentionTags,
      ...
      if ("id" in sendResult && typeof sendResult.id === "string") {
        return sendResult;
      }

      requireAcceptedMessageSend(sendResult);
      const publishRes = sendResult as PublishResult;
      return {
        id: publishRes.eventId || "e".repeat(64),
        pubkey: identity.pubkey,
