...
async function withThreadAux(
  ...
  const [structuralAux, reactions] = await Promise.all([
    ...
    fetchThreadAuxBestEffort("reactions", channelId, async () => {
      // Per instructions: Replace relayClient.fetchAuxEventsByReference with fetchChannelMessages filtered by threadRootId
      const msgs = await fetchChannelMessages(channelId, 200);
      return msgs.filter((m: any) => m.threadRootId === threadRootId) as any;
    }),
  ]);
