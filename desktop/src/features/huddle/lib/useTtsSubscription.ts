...
export function useTtsSubscription(
  ...
  React.useEffect(() => {
    ...
    // The scoped subscription uses limit:0, so the relay sends no historical
    // backlog. Every event delivered is a live message.
    ...
    const handler = ({
      ...
      if (disposed || channelId !== ephemeralChannelId) return;
      if (
    ...
    void simpleSocket
      .subscribeToChannelEvent(ephemeralChannelId, "message_send", handler)
      .then((unsubscribe) => {
        if (disposed) {
          void unsubscribe();
        } else {
      ...
      .catch((error) => {
        if (!disposed) {
          console.error("[huddle] Failed to subscribe to TTS messages:", error);
        }
      });
