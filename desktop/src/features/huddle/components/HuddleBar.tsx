...
export function HuddleBar({
  ...
  React.useEffect(() => {
    ...
    const cleanup = simpleSocket.on("huddle_event", ({ event, channelId }) => {
      if (disposed) return;
      if (channelId !== reactionChannelId) return;
  ...
  React.useEffect(() => {
    ...
    const cleanup = simpleSocket.on("huddle_event", ({ event, channelId }) => {
      ...
      if (channelId !== parentChannelId) return;
      if (parseEphemeralChannelId(event.content) !== reactionChannelId) return;
    ...
    (emoji: string) => {
      ...
      void (async () => {
        ...
        if ("accepted" in result && result.accepted === false) {
          throw new Error(result.message || "Relay rejected huddle reaction.");
        }
      })().catch((error) => {
