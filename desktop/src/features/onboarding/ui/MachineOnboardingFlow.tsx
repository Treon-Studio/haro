...
export function MachineOnboardingFlow({
  ...
  // Direct Enterprise Auth Architecture: Automatically generate/retrieve local identity key
  // so the machine onboarding completes instantly and lands on the Login / Dashboard flow.
  React.useEffect(() => {
    if (page === "identity" && !isPending) {
      setIsPending(true);
      getIdentity()
        .then((identity) => {
        ...
        .catch((err) => {
          console.error("Auto-generating identity failed:", err);
          setIsPending(false);
        });
    }
  }, [page, isPending, complete]);
