...
import { isContextualChannelEvent } from "@/shared/api/haroEventMapper";
...
export function withChannelTagFallback(
  ...
  return getChannelIdFromTags(event.tags)
    ? event
    : isContextualChannelEvent(event)
  ...
  React.useEffect(() => {
    ...
    const syncSubs = async (): Promise<boolean> => {
      ...
      const additions = Array.from(targetIds)
        ...
        .map(async (channelId) => {
          try {
            const dispose = await simpleSocket.subscribeToChannelLive(
              channelId,
              (event) => {
                const contextual = withChannelTagFallback(event, channelId);
                if (contextual) handleIncomingMessage(contextual);
              },
            );
            if (isCancelled) {
              void dispose().catch(() => {});
