...
test("live contextual reaction without h tag inherits its subscription channel", () => {
  const reaction = {
    id: "reaction",
    kind: 7,
    pubkey: "agent",
...
test("live h-less message is rejected instead of inheriting channel context", () => {
  const message = {
    id: "message",
  ...
  assert.equal(withChannelTagFallback(message, "channel-a"), null);
});
