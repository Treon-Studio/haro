...
import {
  hasRemoteAdvanceBeyond,
  publishableContextSignature,
} from "@/features/channels/readState/readStatePublish";
import { setLocalStorageItemWithRecovery } from "@/shared/lib/localStorageQuota";
import { truncatePubkey } from "@/shared/lib/pubkey";
import {
  realtimeSocketClient as simpleSocket,
  type PublishResult,
} from "@/shared/api/realtimeSocketClient";

const CLIENT_ID_KEY_PREFIX = "buzz.nip-rs.client-id";
const SLOT_ID_KEY_PREFIX = "buzz.nip-rs.slot-id";
const DEBOUNCE_MS = 5_000;

type SplitPublishResult = "published" | "retry" | "permanently_unpublishable";

export interface ReadStateRelay {
...
export interface ReadStateCodec {
  decrypt: ReadStateDecrypt;
  encrypt: (plaintext: string) => Promise<string>;
  sign: typeof signRelayEvent;
}

const defaultReadStateCodec: ReadStateCodec = {
  decrypt: nip44DecryptFromSelf,
  encrypt: nip44EncryptToSelf,
  sign: signRelayEvent,
};

function relayRejection(result: RelayEvent | PublishResult): string | null {
  if ("accepted" in result && result.accepted === false) {
    return result.message || "Relay rejected read-state event.";
  }
  return null;
}
...
export class ReadStateManager {
  ...
  private relay: ReadStateRelay;
  private codec: ReadStateCodec;
  ...
  private unsubscribeLive: (() => Promise<void>) | null = null;
  ...
  private pendingConvergence = false;
  private permanentlyOversizeSignature: string | null = null;
  private destroyed = false;
  private parentResolver: ContextParentResolver | null = null;

  constructor(
    pubkey: string,
    relay: ReadStateRelay = simpleSocket,
    codec: ReadStateCodec = defaultReadStateCodec,
  ) {
    this.pubkey = pubkey;
    this.relay = relay;
    this.codec = codec;
  ...
  private advanceContext(
    ...
    if (unixTimestamp <= current) {
      ...
      this.permanentlyOversizeSignature = null;
  ...
  private async fetchAndMerge(): Promise<void> {
    ...
    try {
      events = await this.relay.fetchEvents({
  ...
  private async mergeEvents(events: RelayEvent[]): Promise<void> {
    ...
    for (const event of events) {
      const parsed = await parseReadStateEvent(
        event,
        this.pubkey,
        this.codec.decrypt,
      );
  ...
  private async startLiveSubscription(): Promise<void> {
    try {
      const handleReadStateLive = (event: RelayEvent) => {
        const publishedBeforeMerge = { ...this.lastPublishedContexts };
        void this.mergeEvents([event])
          .then(() => {
            if (this.destroyed) return;
            this.persistLocalState();
            this.notifyListeners();
            if (
              hasRemoteAdvanceBeyond(
                this.pendingSyncedAdvances,
                this.effectiveState,
                publishedBeforeMerge,
              )
            ) {
          ...
          .catch((error) => {
            console.debug("[ReadStateManager] live merge failed:", error);
          });
      };
      const unsub = await this.relay.subscribeLive(
        {
          kinds: [KIND_READ_STATE],
          authors: [this.pubkey],
          "#t": ["read-state"],
  ...
  private async publish(): Promise<void> {
    ...
    const forceConvergence = this.pendingConvergence;
    ...
    if (contexts === null) {
      ...
      const signature = publishableContextSignature(
        this.effectiveState,
        this.publishableContextIds,
      );
      if (this.permanentlyOversizeSignature === signature) return;
      const result = await this.publishSplitSlots(forceConvergence);
      if (result === "published") {
        this.pendingConvergence = false;
      } else if (result === "permanently_unpublishable") {
        this.permanentlyOversizeSignature = signature;
      } else {
        this.schedulePublish();
      }
      return;
    ...
    if (this.extraSlotIds.length > 0) {
      const published = await this.publishOneSlot(this.slotId, contexts);
      if (!published) {
        this.schedulePublish();
        return;
      }
      ...
      if (this.extraSlotIds.length > 0) this.schedulePublish();
      return;
    }

    if (!forceConvergence && this.isIdenticalToLastPublished(contexts)) return;
  ...
  private async publishOneSlot(
    ...
    try {
      ...
      const ciphertext = await this.codec.encrypt(plaintext);
      ...
      const event = await this.codec.sign({
      ...
      const rejection = relayRejection(result);
      if (rejection) {
      ...
      for (const key of Object.keys(contexts)) {
        this.pendingSyncedAdvances.delete(key);
      }
      return true;
    } catch (error) {
   ...
   */
  private async publishSplitSlots(
    forceConvergence = false,
  ): Promise<SplitPublishResult> {
    const slots = this.splitContextsIntoSlots();
    if (slots === null) return "permanently_unpublishable";
    ...
    if (!forceConvergence && this.isIdenticalToLastPublished(unionContexts)) {
      return "published";
    }
    ...
    for (const { slotId, contexts } of slots) {
      if (!(await this.publishOneSlot(slotId, contexts))) {
        allPublished = false;
      }
    }
    return allPublished ? "published" : "retry";
  }
  ...
  private async deleteExtraSlots(): Promise<void> {
    const retainedSlotIds: string[] = [];
    for (const slotId of this.extraSlotIds) {
      try {
        const dTag = `${READ_STATE_D_TAG_PREFIX}${slotId}`;
        const event = await this.codec.sign({
          kind: 5,
          content: "",
          tags: [["a", `${KIND_READ_STATE}:${this.pubkey}:${dTag}`]],
        });
        const result = await this.relay.publishEvent(event);
        const rejection = relayRejection(result);
        if (rejection) {
        ...
        retainedSlotIds.push(slotId);
      }
    }
    this.extraSlotIds = retainedSlotIds;
    saveExtraSlotIds(this.pubkey, retainedSlotIds);
  }

  private async fetchOwnBlobBeforePublish(): Promise<void> {
    try {
      const now = Math.floor(Date.now() / 1_000);
      const since = now - READ_STATE_HORIZON_SECONDS;
      const knownDTags = [this.slotId, ...this.extraSlotIds].map(
        (slotId) => `${READ_STATE_D_TAG_PREFIX}${slotId}`,
      );
      const eventPages = await Promise.all(
        knownDTags.map((dTag) =>
          this.relay.fetchEvents({
            kinds: [KIND_READ_STATE],
            authors: [this.pubkey],
            "#d": [dTag],
            "#t": ["read-state"],
