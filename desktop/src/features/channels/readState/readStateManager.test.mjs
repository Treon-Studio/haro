...
const READ_STATE_PUBKEY = "b".repeat(64);

function readStateEvent({
  clientId = "remote-client",
  contexts = { "channel-remote": 1_700_000_000 },
  createdAt = 1_700_000_100,
  slotId = "remote-slot",
} = {}) {
  return {
    id: "a".repeat(64),
    pubkey: READ_STATE_PUBKEY,
    ...
    content: "encrypted-remote-blob",
    sig: "c".repeat(128),
    plaintext: JSON.stringify({ v: 1, client_id: clientId, contexts }),
  };
}

function readStateCodec() {
  return {
    decrypt: async (ciphertext, event) =>
      event?.plaintext ??
      (ciphertext === "encrypted-remote-blob"
        ? readStateEvent().plaintext
        : (() => {
            throw new Error("unexpected ciphertext");
          })()),
    encrypt: async () => "encrypted-local-blob",
    sign: async (input) => ({
      id: "d".repeat(64),
      pubkey: READ_STATE_PUBKEY,
...
test("initialize fetches and hydrates encrypted read state from another device", async () => {
  globalThis.window.localStorage = makeLocalStorage();
  const remote = readStateEvent();
  const filters = [];
  const relay = {
    ...
    async publishEvent() {
      return { accepted: true, eventId: "f".repeat(64), message: "ok" };
    },
    async subscribeLive(filter) {
      filters.push(filter);
      return async () => {};
    },
  };
  const mgr = new ReadStateManager(READ_STATE_PUBKEY, relay, readStateCodec());

  await mgr.initialize();

  assert.equal(mgr.getEffectiveTimestamp("channel-remote"), 1_700_000_000);
  assert.deepEqual(filters[0].kinds, [30078]);
  assert.deepEqual(filters[0].authors, [READ_STATE_PUBKEY]);
  assert.deepEqual(filters[0]["#t"], ["read-state"]);
  assert.equal(typeof filters[0].since, "number");
  mgr.destroy();
});

test("publishOneSlot sends one encrypted read-state event and no plaintext timestamp", async () => {
  globalThis.window.localStorage = makeLocalStorage();
  const published = [];
  const relay = {
    async fetchEvents() {
  ...
  const codec = readStateCodec();
  codec.decrypt = async () =>
    JSON.stringify({
      v: 1,
      client_id: "remote-client",
      contexts: { "channel-live": 200 },
    });
  const mgr = new ReadStateManager(READ_STATE_PUBKEY, relay, codec);

  await mgr.publishOneSlot("local-slot", { "channel-local": 1_700_000_123 });

  assert.equal(published.length, 1);
  assert.equal(published[0].kind, 30078);
  assert.equal(published[0].content, "encrypted-local-blob");
  assert.equal(published[0].content.includes("1700000123"), false);
  assert.deepEqual(published[0].tags, [
    ["d", "read-state:local-slot"],
    ["t", "read-state"],
  ]);
});

test("publishOneSlot keeps local publish state unchanged when the relay rejects", async () => {
  globalThis.window.localStorage = makeLocalStorage();
  let publishCalls = 0;
  const relay = {
    async fetchEvents() {
    ...
    async publishEvent() {
      publishCalls += 1;
      return { accepted: false, eventId: "d".repeat(64), message: "blocked" };
    },
    async subscribeLive() {
      return async () => {};
    },
  ...
  assert.deepEqual(mgr.lastPublishedContexts, {});
  assert.equal(mgr.maxFetchedCreatedAt, 0);
});

test("deleteExtraSlots publishes NIP-09 address deletions and retains rejected slots", async () => {
  globalThis.window.localStorage = makeLocalStorage();
  const published = [];
  const relay = {
    async fetchEvents() {
    ...
    async publishEvent(event) {
      published.push(event);
      const dCoordinate = event.tags.find((tag) => tag[0] === "a")?.[1];
      return {
        accepted: !dCoordinate.endsWith(":rejected-slot"),
        eventId: event.id,
        message: "blocked",
      };
    },
  ...
  const mgr = new ReadStateManager(READ_STATE_PUBKEY, relay, readStateCodec());
  mgr.extraSlotIds = ["deleted-slot", "rejected-slot"];

  await mgr.deleteExtraSlots();

  assert.equal(published.length, 2);
  assert.ok(published.every((event) => event.kind === 5));
  assert.deepEqual(
    published.map((event) => event.tags),
    [
      [["a", `30078:${READ_STATE_PUBKEY}:read-state:deleted-slot`]],
      [["a", `30078:${READ_STATE_PUBKEY}:read-state:rejected-slot`]],
    ],
  );
  assert.deepEqual(mgr.extraSlotIds, ["rejected-slot"]);
});

test("publish compacts by acknowledging the union before deleting extra slots", async () => {
  globalThis.window.localStorage = makeLocalStorage();
  const mgr = new ReadStateManager(
    READ_STATE_PUBKEY,
    {
      ...
      async publishEvent() {
        return { accepted: true, eventId: "e".repeat(64) };
      },
      async subscribeLive() {
        return async () => {};
      },
  ...
  mgr.extraSlotIds = ["extra-slot"];
  const order = [];
  mgr.fetchOwnBlobBeforePublish = async () => {};
  mgr.currentContexts = () => ({ "channel-union": 200 });
  mgr.publishOneSlot = async () => {
    order.push("publish-union");
    return true;
  };
  mgr.deleteExtraSlots = async () => {
    order.push("delete-extra");
    mgr.extraSlotIds = [];
  };

  await mgr.publish();

  assert.deepEqual(order, ["publish-union", "delete-extra"]);
  mgr.destroy();
});

test("publish keeps extra slots when consolidated union is rejected and retries", async () => {
  globalThis.window.localStorage = makeLocalStorage();
  const mgr = new ReadStateManager(
    READ_STATE_PUBKEY,
    {
  ...
  mgr.publishOneSlot = async () => false;
  mgr.deleteExtraSlots = async () => {
    deleteCalled = true;
  };
  let retries = 0;
  ...
  assert.deepEqual(mgr.extraSlotIds, ["extra-slot"]);
  assert.equal(retries, 1);
  mgr.destroy();
});

test("fetchOwnBlobBeforePublish queries each known slot coordinate within the horizon", async () => {
  globalThis.window.localStorage = makeLocalStorage();
  const filters = [];
  const relay = {
    async fetchEvents(filter) {
    ...
    async publishEvent() {
      return { accepted: true, eventId: "f".repeat(64) };
    },
    async subscribeLive() {
      return async () => {};
    },
  ...
  const mgr = new ReadStateManager(READ_STATE_PUBKEY, relay, readStateCodec());
  mgr.extraSlotIds = ["extra-a", "extra-b"];
...
test("permanently oversize split state is suppressed until state changes", async () => {
  globalThis.window.localStorage = makeLocalStorage();
  const mgr = new ReadStateManager(
    READ_STATE_PUBKEY,
    {
  ...
  mgr.currentContexts = () => null;
  let splitCalls = 0;
  mgr.splitContextsIntoSlots = () => {
    splitCalls += 1;
    return null;
  };
  let retries = 0;
...
test("transient split publish rejection schedules a retry", async () => {
  globalThis.window.localStorage = makeLocalStorage();
  const mgr = new ReadStateManager(
    READ_STATE_PUBKEY,
    {
  ...
  mgr.currentContexts = () => null;
  mgr.splitContextsIntoSlots = () => [
    { slotId: "primary", contexts: { "channel-1": 1 } },
  ];
  mgr.publishOneSlot = async () => false;
  let retries = 0;
  mgr.schedulePublish = () => {
...
test("incoming advanced live read state schedules convergence after persistence", async () => {
  globalThis.window.localStorage = makeLocalStorage();
  let liveHandler;
  const relay = {
    async fetchEvents() {
    ...
    async subscribeLive(_filter, handler) {
      liveHandler = handler;
      return async () => {};
    },
  };
  ...
  const mgr = new ReadStateManager(READ_STATE_PUBKEY, relay, codec);
  mgr.lastPublishedContexts = { "channel-live": 100 };
  let schedules = 0;
  mgr.schedulePublish = () => {
    schedules += 1;
  };
  ...
  assert.equal(mgr.getOwnTimestamp("channel-live"), 200);
