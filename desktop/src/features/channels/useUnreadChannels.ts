...
export function useUnreadChannels(
  ...
  // biome-ignore lint/correctness/useExhaustiveDependencies: pubkey are intentional reset signals
  React.useEffect(() => {
    ...
    void Promise.all(
      toFetch.map(async (channelId): Promise<CatchUpResult> => {
        try {
          ...
          const events: RelayEvent[] = []; // await chatStore.fetchChannelMessages(channelId);
