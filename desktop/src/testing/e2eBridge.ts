...
const mockProfileEvents = new Map<string, RelayEvent>();
...
function getMockProfileEvent(pubkey: string): RelayEvent | null {
  const normalizedPubkey = pubkey.toLowerCase();
  const stored = mockProfileEvents.get(normalizedPubkey);
  if (stored) return stored;

  const profile = mockProfiles.get(normalizedPubkey);
  if (!profile?.has_profile_event) return null;

  return {
    id: `mock-profile-${normalizedPubkey}`,
    pubkey: normalizedPubkey,
    created_at: 1,
    kind: 0,
    tags: [],
    content: JSON.stringify({
      ...
      ...(profile.avatar_url !== null && { picture: profile.avatar_url }),
      ...(profile.about !== null && { about: profile.about }),
      ...(profile.nip05_handle !== null && { nip05: profile.nip05_handle }),
    }),
    sig: "mocksig".repeat(20).slice(0, 128),
  };
}

function recordMockProfileEvent(event: RelayEvent): boolean {
  ...
  const profile: RawProfile = {
    ...
    avatar_url: typeof content.picture === "string" ? content.picture : null,
    about: typeof content.about === "string" ? content.about : null,
    nip05_handle: typeof content.nip05 === "string" ? content.nip05 : null,
    owner_pubkey: existing?.owner_pubkey ?? null,
    is_agent: existing?.is_agent,
    has_profile_event: true,
  };
  mockProfiles.set(pubkey, profile);
  mockProfileEvents.set(pubkey, event);
  applyMockDisplayName(pubkey, displayName);
  return true;
}
...
function sendToMockSocket(args: {
  ...
  if (type === "REQ") {
    ...
    if (filter.kinds?.includes(0)) {
      ...
      for (const author of authors) {
        const profileEvent = getMockProfileEvent(author);
        if (profileEvent) {
          sendWsText(socket.handler, ["EVENT", subId, profileEvent]);
        }
      }
      sendWsText(socket.handler, ["EOSE", subId]);
      return;
  ...
  if (type === "EVENT") {
    ...
    if (event.kind === 0) {
      const accepted = recordMockProfileEvent(event);
      if (accepted) {
      ...
      sendWsText(socket.handler, [
        ...
        event.id,
        accepted,
        accepted ? "" : "Invalid profile content.",
      ]);
      return;
    }
  ...
  const handleMockCommand = async (command: string, payload: unknown) => {
    ...
    switch (command) {
      ...
      case "upload_media_bytes": {
        const descriptor = (
          await resolveMockUploadDescriptors(activeConfig)
        )[0];
        return descriptor
          ? {
              descriptor,
